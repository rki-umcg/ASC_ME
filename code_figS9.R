...
###
parent_folder <- paste(data_path,"Ki67_separation", sep = "")
...
all_data_Ki67pos <- lapply(subfolders, function(folder) {
  file_path <- file.path(folder, "Ki67pos_final_results.xlsx")
  
  if (file.exists(file_path)) {
    df <- read_excel(file_path)
    df[,c(3:15)] <- lapply(df[,c(3:15)], as.numeric)
    df$subfolder <- basename(folder)  # add subfolder name as a new column
    df$type <- "Ki67pos.ASCs"  # add ASC type as a new column
    return(df)
  } else {
    message("No final_results.xlsx found in: ", folder)
    return(NULL)
...
all_data_Ki67neg <- lapply(subfolders, function(folder) {
  file_path <- file.path(folder, "Ki67neg_final_results.xlsx")
  
  if (file.exists(file_path)) {
    df <- read_excel(file_path)
    df[,c(3:15)] <- lapply(df[,c(3:15)], as.numeric)
    df$subfolder <- basename(folder)  # add subfolder name as a new column
    df$type <- "Ki67neg.ASCs"  # add ASC type as a new column
    return(df)
  } else {
    message("No final_results.xlsx found in: ", folder)
    return(NULL)
...
data_merge<- rbind(all_data, all_data_Ki67neg, all_data_Ki67pos) %>%
...
# trim output table
new_output<-predictions_new[,-5] #%>%  select(sample, StudyID,SubjectID, organ, group, dpi, p)
...
plot<-ggplot(plot_data, aes(x = type, y = p)) +
  ...
  xlim("total.ASCs", "Ki67pos.ASCs", "Ki67neg.ASCs")+
  annotate("text", x=1, y=max(plot_data$p)*1.1, label=paste("comparison"), size=5)+
  geom_segment(aes(x = 1.4, y = max(plot_data$p)*1.1, xend = 1.6, yend = max(plot_data$p)*1.1),arrow = arrow(length=unit(0.30,"cm"), type = "closed"))+
  annotate("text", x="Ki67pos.ASCs", y=max(plot_data$p)*1.1, label=paste("p=",format(kt[["plot_data[[4]]"]][3,4], digits=3)), size=5)+
  annotate("text", x="Ki67neg.ASCs", y=max(plot_data$p)*1.1, label=paste("p=",format(kt[["plot_data[[4]]"]][2,4], digits=3)), size=5)
print(plot) 
ggsave(filename = "output/FigS9_plot_all.data_Ki67.png", plot = plot,
       width = 3000, height = 1920, units = "px")

## plot Ki67 + and - by dpi group
merge_dpi<-left_join(plot_data, joined_data[,c(1,32)], by = "sample")
plot_sep<-merge_dpi %>% 
  filter(type=="Ki67pos.ASCs") %>%
  filter(Study.Time.Collected<181) %>%
  mutate(across(Study.Time.Collected, ~ ifelse(.x < 0, 0, .x)))
comp_Ki67pos_group<-plot_sep %>%
...
plot_sep2<-merge_dpi %>% 
  filter(type=="Ki67neg.ASCs") %>%
  filter(Study.Time.Collected<181) %>%
  mutate(across(Study.Time.Collected, ~ ifelse(.x < 0, 0, .x)))
comp_Ki67neg_group<-plot_sep2 %>%
  mutate(dpi_group = cut(Study.Time.Collected,  
                         breaks = c(-Inf, 1, 4.9, 10, 19, 50, 99, 500),  # Define the breaks for the age groups
                         labels = c("BL_neg", "excl", "VE_neg", "E_neg", "I_neg", "L_neg", "VL_neg"))) %>%# Define the labels for the age groups
  filter(!(dpi_group=="excl"))

plot_comp.data<-rbind(comp_Ki67pos_group, comp_Ki67neg_group)
...
plot_comp <- ggplot(plot_comp.data, aes(x = dpi_group, y = p, color = group_type)) +
  ...
  scale_color_manual(values = c("Ki67+ ASCs" = "blue", "Ki67- ASCs" = "red")) + 
  ylim(5, 95) +
  xlim("BL_pos", "BL_neg", "VE_pos", "VE_neg", "E_pos", "E_neg",
       "I_pos", "I_neg", "L_pos", "L_neg", "VL_pos", "VL_neg")
print(plot_comp)
ggsave("output/FigS9_plot_Ki67_kinetic.png",
       width = 3000, height = 1920, units = "px")
...
#dt[["P.adjusted"]][1]
output_stats<-as.data.frame(cbind(dt[["comparisons"]],dt[["P.adjusted"]]))
write_xlsx(output_stats, "output/stats_Ki67.xlsx")
