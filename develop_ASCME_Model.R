...
# Path to the parent folder containing the output from the cytometry analysis (especially final_results.xlsx file)
  # insert your respective location of this data
...
write_xlsx(data, "output/data.unimputed.xlsx")

#use multiple imputation (with random forest) to complete missing training data
dataformerging <- data %>% 
  select(sample,count,source_file,SubjectID)
dataforimp <- data %>% 
  select(-c(count,source_file,SubjectID))

per_miss<-countNA(dataforimp)/(ncol(dataforimp)*nrow(dataforimp))

iter=20
imp2025 <- mice(dataforimp, method = "rf", m = iter, seed = 557)
...
prediction_marker<-c(colnames(dataforimp[,c(2:9)]))
...
write_xlsx(data.imputed_mean, "output/data.imputed_mean.xlsx")

prediction_marker_age<-c(colnames(data.imputed[[1]][,c(2:9)]),"Age")
...
predictions_mean <- cbind(dataformerging,dataforimp[,c(13,14,12)], prediction_all[[1]][, 1:2], mean_predictions)
# trim output table
...
plot_HC_kinetic<-ggplot(model_HC_kinetic, aes(x=dpi, y=p)) +
  ...
  #geom_smooth(method = "gam", n=10, color = "red", se = TRUE , level = 0.99) + #add regression line /w confidence interval
...
ggsave(filename = "output/Fig3A_train_final_ASCME.png", width = 3000, height = 1920, units = "px")
...
iter=20
data.norm<-cbind(data.imputed_mean[,c(1,4:11)],dataforimp[,c(10:16)])
...
ggsave(filename = "output/Fig3D_validation_final_ASCME.png", width = 3000, height = 1920, units = "px")


####
# plots for linked data points and dpi groups (training data)
...
plot_HC_long<-ggplot(model_HC_long, aes(x=dpi, y=p, group=SubjectID, color=SubjectID)) +
  ...
  theme(legend.position = "none",
        ...
        plot.title = element_text(size = 15,face = "bold"))
print(plot_HC_long)
ggsave("output/Fig3B_plot_HC_long_final.png", plot = plot_HC_long, width = 3000, height = 1920, units = "px")
...
plot_HC_dpi_gp<-ggplot(model_HC_dpi_gp, aes(x = dpi_group, y = p)) +
  ...
  geom_segment(aes(x = 1.5, y = 144, xend = 1.7, yend = 144),arrow = arrow(length=unit(0.30,"cm"), type = "closed"))+
  annotate("text", x="very early", y=145, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][1,4], digits=3)), size=5)+
  annotate("text", x="early", y=145, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][2,4], digits=3)), size=5)+
  annotate("text", x="intermediate", y=145, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][3,4], digits=3)), size=5)+
  annotate("text", x="late", y=145, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][4,4], digits=3)), size=5)+
  annotate("text", x="very late", y=145, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][5,4], digits=3)), size=5)+
  annotate("text", x="baseline", y=135, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][5,4], digits=3)), size=5)+
  annotate("text", x="very early", y=135, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][9,4], digits=3)), size=5)+
  annotate("text", x="early", y=135, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][12,4], digits=3)), size=5)+
  annotate("text", x="intermediate", y=135, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][14,4], digits=3)), size=5)+
  annotate("text", x="late", y=135, label=paste("p=",format(ANOVA_dev[["model_HC_dpi_gp[[10]]"]][15,4], digits=3)), size=5)+
  geom_segment(aes(x = 5.5, y = 134, xend = 5.3, yend = 134),arrow = arrow(length=unit(0.30,"cm"), type = "closed"))+
  annotate("text", x="very late", y=135, label=paste("comparison"), size=5)
print(plot_HC_dpi_gp) 
ggsave("output/Fig3C_plot_HC_dpi_gp_final.png", plot = plot_HC_dpi_gp, width = 3000, height = 1920, units = "px")

####
# plots for linked data points and dpi groups (validation data)

# check longitudinal/kinetic samples
model_new_long<-new_output %>%
  filter((group == "HC")|(group == "HC_CoV")|(group == "HC_Inf")|(group == "HC_Vac")|(group == "HC_SCT")) %>%
  mutate(across(dpi, ~ ifelse(.x < 0, 0, .x)))
#filter(dpi==0|dpi>4)

#Identify non-unique SubjectID values in model_HC_long
non_unique_subjects <- model_new_long %>%
  count(SubjectID) %>%  # Count occurrences of each SubjectID
  filter(n > 1) %>%  # Keep only SubjectID with more than 1 occurrence
  pull(SubjectID)  # Extract the SubjectID values
#filter out non-unique samples
model_new_long <- model_new_long %>%
  filter(SubjectID %in% non_unique_subjects)

plot_new_long<-ggplot(model_new_long, aes(x=dpi, y=p, group=SubjectID, color=SubjectID)) +
  geom_point(shape=1) + #none-filled circles
  geom_line(size=0.25) + #, aes(color=SubjectID)) +
  scale_color_viridis(discrete = TRUE, option = "H") +
  labs(title = "linked longitudinal data points",
  ...
  theme(legend.position = "none",
        ...
        plot.title = element_text(size = 15,face = "bold"))
print(plot_new_long)
ggsave("output/Fig3D_plot_new_long_final.png", plot = plot_new_long, width = 3000, height = 1920, units = "px")

#by dpi groups
model_new_dpi_gp<-new_output %>%
  mutate(dpi_group = cut(dpi,  
                         breaks = c(-Inf, 1, 4.9, 10, 19, 50, 99, 500),  # Define the breaks for the age groups
                         labels = c("baseline", "excl", "very early", "early", "intermediate", "late", "very late"))) %>%# Define the labels for the age groups
  filter(!(dpi_group=="excl"))

ANOVA_dev<- lm(model_new_dpi_gp[[5]] ~ model_new_dpi_gp[[8]])  |> aov() |> TukeyHSD()

plot_new_dpi_gp<-ggplot(model_new_dpi_gp, aes(x = dpi_group, y = p)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(color = as.factor(model_new_dpi_gp[[2]])), width = 0.25, alpha = 1, size = 2,shape=1) +
  theme_bw() +
  labs(title = "one-way ANOVA with Tukey correction",
       x = "immune response stage",
       y = "predicted DPI",
  ...
  annotate("text", x="baseline", y=145, label=paste("comparison"), size=5)+
  geom_segment(aes(x = 1.5, y = 144, xend = 1.7, yend = 144),arrow = arrow(length=unit(0.30,"cm"), type = "closed"))+
  annotate("text", x="very early", y=145, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][1,4], digits=3)), size=5)+
  annotate("text", x="early", y=145, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][2,4], digits=3)), size=5)+
  annotate("text", x="intermediate", y=145, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][3,4], digits=3)), size=5)+
  annotate("text", x="late", y=145, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][4,4], digits=3)), size=5)+
  annotate("text", x="very late", y=145, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][5,4], digits=3)), size=5)+
  annotate("text", x="baseline", y=135, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][5,4], digits=3)), size=5)+
  annotate("text", x="very early", y=135, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][9,4], digits=3)), size=5)+
  annotate("text", x="early", y=135, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][12,4], digits=3)), size=5)+
  annotate("text", x="intermediate", y=135, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][14,4], digits=3)), size=5)+
  annotate("text", x="late", y=135, label=paste("p=",format(ANOVA_dev[["model_new_dpi_gp[[8]]"]][15,4], digits=3)), size=5)+
  geom_segment(aes(x = 5.5, y = 134, xend = 5.3, yend = 134),arrow = arrow(length=unit(0.30,"cm"), type = "closed"))+
  annotate("text", x="very late", y=135, label=paste("comparison"), size=5)
print(plot_new_dpi_gp) 
ggsave("output/Fig3D_plot_new_dpi_gp_final.png", plot = plot_new_dpi_gp, width = 3000, height = 1920, units = "px")


###
### save data parts for down-stream application
###
write_xlsx(joined_data, path = "output/final_joined_data.xlsx")
write_xlsx(data.norm, path = "output/final_data.norm.xlsx")
write_xlsx(model_output, path = "output/final_model_output.xlsx")
saveRDS(rf_ascmi, file = "output/final_model.rds")
