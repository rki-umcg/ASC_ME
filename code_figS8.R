...
### new: downsample training and val datasets

train_df2 <- train_df %>%
  group_by(Study.Accession) %>%
  sample_n(size = min(table(train_df$Study.Accession))) %>%
  ungroup()
...
write_xlsx(data, "output/data.unimputed_downsample.xlsx")
...
write_xlsx(data.imputed_mean, "output/data.imputed_mean_downsample.xlsx")
...
ggsave(filename = "output/train_downsample.png", width = 2571, height = 1615, units = "px")


## downsample val data
val_df2 <- val_df %>%
  group_by(Study.Accession) %>%
  sample_n(size = min(table(val_df$Study.Accession))) %>%
  ungroup()
...
ggsave(filename = "output/val_downsample.png", width = 2571, height = 1615, units = "px")
